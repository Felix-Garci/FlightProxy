...
public:
  std::vector<uint8_t>
  encode(std::unique_ptr<const FlightProxy::Core::MspPacket> packet) override {
...
private:
  ...
  std::function<void(std::unique_ptr<const FlightProxy::Core::MspPacket>)>
      onPacketHandler_;
  ...
  void parse(uint8_t byte) {
    ...
    case ParseState::CHECKSUM:
      // FP_LOG_D("MspDecoder", "Checksum de MSP procesado recibido: 0x%02X,
      // calculado: 0x%02X", byte, calculatedChecksum_);
      if (byte == calculatedChecksum_) {
        // FP_LOG_D("MspDecoder", "Checksum correcto");
        if (onPacketHandler_) {
          // FP_LOG_D("MspDecoder", "Llamando al handler de paquete MSP");
          onPacketHandler_(
              std::make_unique<FlightProxy::Core::MspPacket>(workingPacket_));
        }
      }
