 ...
 * Genera paquetes de 32 bytes:
 * [0x20][0x40][CH1_L][CH1_H]...[CH14_H][CHK_L][CHK_H]
 */
class IbusEncoder : public IEncoderT<FlightProxy::Core::IBUSPacket> {
public:
  std::vector<uint8_t>
  encode(std::unique_ptr<const FlightProxy::Core::IBUSPacket> packet) override {
 ...
 * Máquina de estados robusta para detectar tramas válidas de 32 bytes empezando
 * por 0x20 0x40.
 */
...
private:
  ...
  uint16_t current_checksum_sum_ =
      0; // Acumulador para calcular el checksum sobre la marcha
  uint8_t payload_index_ = 0;
  uint8_t raw_payload_[Detail::IBUS_PAYLOAD_SIZE]; // Buffer temporal para los
                                                   // datos crudos
  uint16_t received_checksum_ = 0;

  std::function<void(std::unique_ptr<const FlightProxy::Core::IBUSPacket>)>
      onPacketHandler_;
  ...
  void parse(uint8_t byte) {
    ...
    case ParseState::CMD:
      if (byte == Detail::IBUS_CMD_SERVO) {
        ...
        state_ =
            (byte == Detail::IBUS_HEADER) ? ParseState::CMD : ParseState::IDLE;
    ...
    case ParseState::CHECKSUM_H:
      ...
      if (received_checksum_ == (uint16_t)(0xFFFF - current_checksum_sum_)) {
        if (onPacketHandler_) {
          ...
          for (size_t i = 0; i < FlightProxy::Core::IBUSPacket::NUM_CHANNELS;
               ++i) {
            packet->channels[i] =
                raw_payload_[i * 2] |
                (static_cast<uint16_t>(raw_payload_[i * 2 + 1]) << 8);
          }
