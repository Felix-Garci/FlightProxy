...
SimpleTCP::SimpleTCP(const char *ip, uint16_t port)
  ...
  if (ip != nullptr) {
    ...
    // Asegura que la cadena siempre termine en nulo, incluso si la IP es muy
    // larga
    ip_[sizeof(ip_) - 1] = '\0';
  } else {
...
SimpleTCP::~SimpleTCP() { FP_LOG_I(TAG, "Canal destruido."); }
...
void SimpleTCP::open() {
  ...
  if (m_sock == -1) {
    ...
    if (ip_[0] == '\0' || port_ == 0) {
      FP_LOG_E(
          TAG,
          "No se puede abrir: IP o puerto no configurados para el cliente.");
      return;
    }
  ...
  if (!self_keep_alive) {
    ...
    FP_LOG_E(TAG, "open() falló: el objeto está siendo destruido (no se pudo "
                  "obtener shared_ptr)");
  ...
  BaseType_t result =
      xTaskCreate(eventTaskAdapter, // La función adaptadora estática
...
void SimpleTCP::send(const uint8_t *data, size_t len) {
  ...
  while (total_sent < len) {
    ...
    if (sent_now < 0) {
      FP_LOG_E(TAG, "Canal (socket %d): Error en send(): %d. Abortando envío.",
               m_sock, errno);
...
void SimpleTCP::eventTask(std::shared_ptr<SimpleTCP> *self_ptr_on_heap) {
  ...
  while (true) {
    ...
    if (len > 0) {
      ...
      FP_LOG_E(TAG,
               "Error en recv(): %d. Error en la conexion o que se llamo a "
               "this->close()",
               errno);
      break;
    }
