...
SimpleUDP::SimpleUDP(uint16_t port)
    : m_port(port), m_last_sender_len(sizeof(m_last_sender_addr)),
      m_has_last_sender(false), eventTaskHandle_(nullptr),
...
SimpleUDP::~SimpleUDP() { FP_LOG_I(TAG, "Canal destruido."); }
...
void SimpleUDP::open() {
  ...
  if (m_sock == -1) {
    FP_LOG_I(TAG, "Modo servidor UDP: Intentando escuchar en el puerto %u...",
             m_port);
...
void SimpleUDP::send(const uint8_t *data, size_t len) {
  ...
  if (!m_has_last_sender) {
    FP_LOG_W(TAG, "Canal: Intento de envío sin un destinatario (aún no se ha "
                  "recibido nada).");
    return;
  }
  ...
  int sent_now =
      ::sendto(m_sock, data, len, 0, (struct sockaddr *)&m_last_sender_addr,
               m_last_sender_len);
