 ...
 */
template <typename T> static const void *getTypeID() {
...
private:
  ...
  struct SlotBase {
    ...
    void updateStats() {
      ...
      if (last_update.time_since_epoch().count() > 0) {
        ...
        // Evitamos división por cero si las actualizaciones son excesivamente
        // rápidas
   ...
   */
  template <typename T> struct TypedSlot : SlotBase {
    T data;

    TypedSlot(T defaultVal) : data(std::move(defaultVal)) {}

    // Implementación que devuelve el ID único de T
    const void *getActualTypeID() const override { return getTypeID<T>(); }
  };
  ...
  std::shared_ptr<TypedSlot<T>> getOrCreateSlot(DataID id) {
    ...
    if (it != m_storage.end()) {
      // --- EL CAJÓN YA EXISTE ---
      // Validamos que el tipo solicitado (T) coincida con el tipo creado
      // originalmente.
      if (it->second->getActualTypeID() != getTypeID<T>()) {
        throw std::runtime_error("Error de tipo (No-RTTI) en DataID: " +
                                 std::to_string(id));
      }
...
public:
  ...
  /**
   * @brief Obtiene la frecuencia estimada actual (en Hz) con decaimiento
   * natural. Si el productor se detiene, este valor bajará progresivamente
   * hacia 0.
   */
  double getFrequency(DataID id) {
    ...
    // 1. Si nunca se ha actualizado o solo una vez (no hay periodo aún),
    // devolvemos 0 o la inicial.
    if (it->second->frequency_hz <= 0.0 ||
        it->second->last_update.time_since_epoch().count() == 0) {
    ...
    // Si es MENOR, significa que estamos "llegando tarde" y la frecuencia
    // efectiva está bajando.
    return std::min(it->second->frequency_hz, estimated_freq_now);
  }

  template <typename T> std::function<void(T)> registrarProductor(DataID id) {
  ...
  template <typename T> std::function<T(void)> registrarConsumidor(DataID id) {
    std::shared_ptr<TypedSlot<T>> slot = getOrCreateSlot<T>(id);
