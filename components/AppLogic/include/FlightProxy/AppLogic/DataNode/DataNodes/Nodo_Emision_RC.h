...
namespace DataNode {
namespace DataNodes {
class Nodo_Emision_RC : public IDataNodeBase,
                        public std::enable_shared_from_this<Nodo_Emision_RC> {
private:
  ...
  void onCanalCerrado() { m_channelRCData.reset(); }

public:
  Nodo_Emision_RC(std::shared_ptr<Core::Channel::IChannelT<Core::MspPacket>>
                      virtualChannelRCData,
                  std::function<Core::RCData()> consumidorRCData)
      : m_channelRCData(virtualChannelRCData), m_consumidor(consumidorRCData) {
    m_channelRCData->onPacket =
    ...
    m_channelRCData->onClose = [this]() { this->onCanalCerrado(); };
  }
