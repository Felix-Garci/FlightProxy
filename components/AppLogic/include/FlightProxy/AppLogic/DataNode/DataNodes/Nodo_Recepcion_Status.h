...
namespace DataNode {
namespace DataNodes {
class Nodo_Recepcion_Status
    : public IDataNodeBase,
      public std::enable_shared_from_this<Nodo_Recepcion_Status> {
private:
  ...
  void onRespuestaRecibida(std::unique_ptr<const Core::MspPacket> pkt) {
    ...
    if (pkt->payload.size() >= 16) {
      datos_status.cycleTime =
          static_cast<uint16_t>(pkt->payload[0] | (pkt->payload[1] << 8));
      datos_status.i2c_errors =
          static_cast<uint16_t>(pkt->payload[2] | (pkt->payload[3] << 8));
      datos_status.sensors =
          static_cast<uint16_t>(pkt->payload[4] | (pkt->payload[5] << 8));
      datos_status.boxModeFlags = static_cast<uint32_t>(
          pkt->payload[6] | (pkt->payload[7] << 8) | (pkt->payload[8] << 16) |
          (pkt->payload[9] << 24));
      datos_status.currentProfileIndex = pkt->payload[10];
      datos_status.averageSystemLoadPercent =
          static_cast<uint16_t>(pkt->payload[11] | (pkt->payload[12] << 8));
      datos_status.armingFlags =
          static_cast<uint16_t>(pkt->payload[13] | (pkt->payload[14] << 8));
  ...
  void onCanalCerrado() { m_channelStatusData.reset(); }

public:
  Nodo_Recepcion_Status(
      std::shared_ptr<Core::Channel::IChannelT<Core::MspPacket>>
          virtualChannelStatusData,
      std::function<void(Core::StatusData)> productorStatusData)
      : m_channelStatusData(virtualChannelStatusData),
        m_productor(productorStatusData) {
    m_channelStatusData->onPacket =
        [this](std::unique_ptr<const Core::MspPacket> pkt) {
          this->onRespuestaRecibida(std::move(pkt));
        };

    // También nos suscribimos al cierre
    m_channelStatusData->onClose = [this]() { this->onCanalCerrado(); };
  }
  void transact() override {
    if (!m_channelStatusData)
      return;
    if (!m_esperandoRespuesta) {
      ...
      auto paqueteSolicitud = std::make_unique<Core::MspPacket>(
          '<', Core::Protocol::MSP_STATUS_DATA,
          payload); // 105 es el comando MSP para Status
      ...
      // FP_LOG_W("Nodo_Recepcion_IMU", "Aún esperando respuesta de datos IMU,
