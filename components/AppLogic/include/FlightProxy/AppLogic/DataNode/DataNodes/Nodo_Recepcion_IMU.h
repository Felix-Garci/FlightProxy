...
private:
  ...
  void onCanalCerrado() { m_channelIMUData.reset(); }
...
public:
  Nodo_Recepcion_IMU(std::shared_ptr<Core::Channel::IChannelT<Core::MspPacket>>
    ...
    m_channelIMUData->onClose = [this]() { this->onCanalCerrado(); };
  }
  void transact() override {
    ...
    if (!m_esperandoRespuesta) {
      ...
      auto paqueteSolicitud = std::make_unique<Core::MspPacket>(
          '<', Core::Protocol::MSP_IMU_DATA,
          payload); // 105 es el comando MSP para IMU
