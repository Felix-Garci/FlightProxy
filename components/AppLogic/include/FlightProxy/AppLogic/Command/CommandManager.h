...
class CommandManager
    : public std::enable_shared_from_this<CommandManager<PacketT>> {
public:
  using SenderFunc =
      std::function<bool(uint32_t, std::unique_ptr<const PacketT> packet)>;
  SenderFunc responsehandler;

  CommandManager(size_t queueSize = 5) {
    packetQueue_ =
        Core::OSAL::OSALFactory::createQueue<Core::PacketEnvelope<PacketT>>(
            queueSize);
  }

  ~CommandManager() { stop(); }

  // Método público para recibir paquetes desde CUALQUIER sitio (Agregador
  // incluido)
...
private:
  std::unique_ptr<Core::OSAL::IQueueT<Core::PacketEnvelope<PacketT>>>
      packetQueue_;
  ...
  void eventLoop() {
    ...
    // Usamos un timeout razonable (ej. 1s) para poder comprobar isRunning_
    // periódicamente
    while (isRunning_) {
  ...
  void processContext(uint32_t channelId,
                      std::unique_ptr<const PacketT> packet) {
    ...
    if (it != commandMap_.end()) {
      ...
      FP_LOG_W("skdhfj", "comando no encontrado recivimos msg de %d",
               packet->command);
    }
  }
};
