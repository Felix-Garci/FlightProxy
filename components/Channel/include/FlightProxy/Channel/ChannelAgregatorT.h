...
class ChannelAgregatorT
    : public std::enable_shared_from_this<ChannelAgregatorT<PacketT>> {
public:
  ChannelAgregatorT() : m_mutex(Core::OSAL::OSALFactory::createMutex()) {}
  ~ChannelAgregatorT() {}

  void addChannel(
      std::shared_ptr<FlightProxy::Core::Channel::IChannelT<PacketT>> channel) {
    ...
    channel->onPacket = [this, myId](std::unique_ptr<const PacketT> packet) {
      // aqui lo que pasa es que tenemos que sacar el puntero crudo para pasarlo
      // por la cola si no entre en la cola lo eliminas para evitar fugas
      if (onPacketFromAnyChannel) {
        // FP_LOG_I("AGREG", "Paquete recibido en agregador con command %d",
        // packet->command);
        ...
        if (!enqueued) {
          FP_LOG_W("AGREG", "Fallo al encolar paquete desde canal borramos ptr "
                            "para evitar fuga");
  ...
  std::function<bool(const Core::PacketEnvelope<PacketT> &)>
      onPacketFromAnyChannel;

private:
  std::atomic<uint32_t> nextChannelId_{1};
  std::unique_ptr<Core::OSAL::IMutex> m_mutex;
  std::map<uint32_t,
           std::shared_ptr<FlightProxy::Core::Channel::IChannelT<PacketT>>>
      channelsById_;
};
